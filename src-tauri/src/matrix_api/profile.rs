...
use shared::{
    ...
    synth::{ProfileAudio, SonicSignature},
};
...
pub async fn send_user_to_frontend(
    handle: &AppHandle,
    client: &Client,
    ...
    let (custom_properties, sonic_signature) =
        get_custom_fields(client, user_id.to_owned(), default_audio).await;
    ...
    send_user_profile_update(handle, update.clone())?;
    ...
    spawn_blocking(move || {
        ...
        update.custom_properties.audio = audio;

        if let Err(e) = send_user_profile_update(&handle, update) {
...
pub async fn client_user_profile_event_handle(
    ...
    if let Err(e) = send_user_to_frontend(&handle, &client, &default_audio).await {
    ...
    tokio::spawn(async move {
        ...
        if should_emit
            && let Err(e) =
                send_user_to_frontend(&handle_clone, &client_clone, &default_audio).await
        {
...
pub async fn get_custom_fields(
    ...
    user_id: OwnedUserId,
    default_audio: &ProfileAudio,
) -> (CustomProperties, SonicSignature) {
    let account = client.account();
    let derived = CustomProperties::from_user_id(user_id.as_str(), default_audio.clone());
    ...
    let sonic_signature = sonic_result
        ...
        .map(|v| v.value().to_string())
        .and_then(|v| serde_json::from_str(&v).ok())
        .unwrap_or_else(|| SonicSignature::from_user_id(user_id.as_str()));

    (
        CustomProperties {
            banner_color,
            name_color,
            audio: default_audio.clone(),
        },
        sonic_signature,
    )
}
