...
pub fn MuteMenuPopup() -> impl IntoView {
    ...
    let devices = move || {
        ...
        let active_id = devices.active_input_device_id.clone();

        let mut items: Vec<(String, String)> = devices
            .input_devices
            .iter()
        ...
        if let Some(id) = devices.default_input_device_id.clone() {
                    ...
                    <button
                        ...
                        on:click=move |_| {
                            if !id_clone.is_empty() {
                                let _ = set_input_device(id_clone.clone());
                            }
                        }
                    >
                        <span class="text-muted flex-shrink-0">
                            <Icon icon=MICROPHONE size="15px" weight=IconWeight::Fill />
                        </span>
            ...
            <div class="bg-(--ui-solid-bg) border border-(--tile-border-color) rounded-(--floating-border-radius) overflow-hidden w-[260px]">
                ...
                >
                    ...
                    <span class="text-xs text-muted truncate mt-0.5">
                        {move || {
                            let devices = app_state.audio_devices.get();
                            if let Some(active_id) = &devices.active_input_device_id {
                                devices
                                    .input_devices
                                    .iter()
